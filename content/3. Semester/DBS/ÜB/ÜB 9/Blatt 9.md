...
# Aufgabe 9-2: **Ableitung eines ER-Diagramms** [[Uni2/Semester/3. Semester/DBS/Erklärungen/ER-Modell/ER-Modell|ER-Modell]]
...
- **Verkauf**: Keine eigenständige Entität, sondern dargestellt als Beziehung mit Attributen Datum, Anzahl, Betrag zwischen Artikel und Angestellter.

### Beziehungen

- **Angestellter und Abteilung**: 1:n. Jeder Angestellte ist einer Abteilung zugeordnet. Eine Abteilung hat einen Angestellten als Leiter (1:1).
...
1. **Schule**: Diese Entität wird durch die Attribute `Name` und `Adresse` charakterisiert. Der `Name` der Schule könnte hier als Primärschlüssel fungieren, da angenommen wird, dass jeder Name einzigartig ist.

2. **Klasse**: Jede `Klasse` gehört zu genau einer `Schule`, was durch die Beziehung `Gehört zu` ausgedrückt wird. Die `Klasse` wird durch die Attribute `ZimmerNr` und `NameLehrer` definiert. Die `ZimmerNr` könnte in Verbindung mit dem Schulnamen als zusammengesetzter Primärschlüssel dienen, da Zimmernummern nur innerhalb einer Schule eindeutig sein müssen.
...
Die Beziehungen zwischen `Geschenken` und `Kindern` sowie `Geschenken` und `Spendern` sind ebenfalls als n:m charakterisiert, was bedeutet, dass ein `Kind` mehrere `Geschenke` erhalten und ein `Geschenk` von mehreren `Spendern` geschenkt werden kann.

In der Klausur sollte auch auf die Nutzung von Pfeilnotation hingewiesen werden, die die Kardinalität der Beziehung angibt. Ebenfalls wichtig ist der Hinweis auf die Notwendigkeit, die Primärschlüssel-Attribute zu unterstreichen, was im vorliegenden Diagramm noch umgesetzt werden müsste. Die Eindeutigkeit der `Namen` in jeder Entität ist eine spezifische Annahme dieses Modells und muss im realen Einsatz durch entsprechende Geschäftsregeln oder Datenbankconstraints sichergestellt werden.
